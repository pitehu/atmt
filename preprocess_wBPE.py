    ...
    def make_split_datasets_bpe(lang, dictionary):
        ...
        if args.tiny_train_prefix is not None:
            make_binary_dataset(args.tiny_train_prefix + '.' + lang, os.path.join(args.dest_dir, 'tiny_train.' + lang+'bpe'),
                                dictionary)
        if args.valid_prefix is not None:
            make_binary_dataset(args.valid_prefix + '.' + lang, os.path.join(args.dest_dir, 'valid.' + lang+'bpe'),
                                dictionary)
        if args.test_prefix is not None:
...
#    subword-nmt apply-bpe -c './data/en-fr/preprocessed/en.out' -i './data/en-fr/preprocessed/train.en' -o './data/en-fr/preprocessed/train.en.bpe'
#
#
#    command_learn_bpe_tgt='subword-nmt learn-bpe -s '+str(args.num_words_src)+' -i '+args.train_prefix + '.' + args.source_lang+' -o '+os.path.join(args.dest_dir, 'dictbpe.' + args.source_lang) 
#    
#    subword-nmt learn-bpe -s 4000 -i './data/en-fr/preprocessed/train.en' -o './data/en-fr/preprocessed/en.out'
#    subword-nmt apply-bpe -c './data/en-fr/preprocessed/en.out' -i './data/en-fr/preprocessed/train.en' -o './data/en-fr/preprocessed/train.en.bpe'
#
#
#
